Create one horizontal look-direction strip for Codex pet `duo-plus`, atlas row 10.

REPAIR PRIORITY: the previous coherent row failed deterministic final-cell edge QA because direction `337.5` had 44 visible pixels in the protected edge band. Keep every pose, especially `315` and `337.5`, noticeably narrower and centered with generous pure-cyan padding on both sides. Do not push the body or either wing toward a slot edge. Preserve one consistent, slightly more compact scale across all eight poses while keeping the feet and lower belly anchored.
